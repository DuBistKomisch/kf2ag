...
function filter()
{
  ...
  if (isNaN(Number($('#filter').val())))
  {
    ...
    get_filter('http://jakebarnes.com.au/ISteamUser/ResolveVanityURL/v0001/?key=80425AE7E1845E2B033ECA38E8F7BCBE&vanityurl=' + $('#filter').val(), 'Resolving username...', function (data)
    {
      ...
      {
        get_filter('http://jakebarnes.com.au/ISteamUserStats/GetUserStatsForGame/v0002/?appid=232090&key=80425AE7E1845E2B033ECA38E8F7BCBE&steamid=' + data.response.steamid, 'Fetching user stats...', function (data)
        {
...
function processAchievements(list, base)
{
  ...
  for (var i = 0; i < list.length; i++)
  {
    var $achievement = $('<article id="' + list[i].id + '"><div><img src="http://cdn.akamai.steamstatic.com/steamcommunity/public/images/apps/232090/' + list[i].icon + '.jpg" alt="icon" /><h3><a href="#' + list[i].id + '">' + list[i].name + '</a></h3><p>' + list[i].description + '</p><p><span class="tag">' + list[i].rate + '</span></p></div></article>');
    // perk
